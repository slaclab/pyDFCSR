...
def _safe_savgol_1d(arr, window_length, polyorder, axis=0):
    """Savitzky–Golay with guardrails: handles NaN/Inf, odd/size limits, and falls back on failure."""
    ...
    w = int(window_length)
    if w <= 1 or n <= 2:
        return arr  # too small to smooth
    if w > n:
        w = n if (n % 2 == 1) else n - 1
    if w < 3:  # still too small
        return arr
    if w % 2 == 0:
        w -= 1
    if polyorder >= w:
    ...
    try:
        return savgol_filter(arr, window_length=w, polyorder=polyorder, axis=axis, mode='interp')
    except Exception:
        return arr


def _safe_savgol_2d(arr, window_length, polyorder):
    """Apply _safe_savgol_1d along both axes (0 then 1)."""
    out = _safe_savgol_1d(arr, window_length, polyorder, axis=0)
    out = _safe_savgol_1d(out, window_length, polyorder, axis=1)
    return out
...
class DF_tracker:
    ...
    def set_absolute_bounds(self, bounds):
        """bounds=(xmin, xmax, zmin, zmax) in LAB frame; None to clear."""
    ...
    def get_DF(self, x, z, px, pz, t):
        ...
        # Use a boolean mask; guard empty slice or sigma_z == 0
        if sigma_z == 0:
            slice_sigX = sigma_x
        else:
            mask = (np.abs(z) < 0.1 * sigma_z)
            if np.any(mask):
                slice_sigX = np.std(x[mask])
                if slice_sigX == 0:
                    slice_sigX = sigma_x
            else:
                slice_sigX = sigma_x
        ...
        # compute bounds for deposition grid
        if self.absolute_bounds is not None:
            xmin, xmax, zmin, zmax = map(float, self.absolute_bounds)
        else:
            xmin = self.xmean - self.xlim * sigma_x
            xmax = self.xmean + self.xlim * sigma_x
            zmin = self.zmean - self.zlim * sigma_z
            zmax = self.zmean + self.zlim * sigma_z
        
        x_grids = np.linspace(xmin, xmax, xbins_t)
        z_grids = np.linspace(zmin, zmax, zbins_t)
        
        density = histogram_cic_2d(
            q1=x, q2=z, w=np.ones(x.shape),
            nbins_1=xbins_t, bins_start_1=xmin, bins_end_1=xmax,
            nbins_2=zbins_t, bins_start_2=zmin, bins_end_2=zmax
        )
        ...
        density = _safe_savgol_2d(density, filter_window, self.filter_order)
        vx       = _safe_savgol_2d(vx,       filter_window, self.filter_order)


        dsum = np.trapz(np.trapz(density, x_grids, axis=0), z_grids)
        if not np.isfinite(dsum) or dsum <= 0:
            # fall back: keep density as-is (or set a tiny uniform baseline if you prefer)
            dsum = 1.0
        density /= dsum
        ...
        density_x = _safe_savgol_2d(density_x, filter_window, self.filter_order)
        density_z = _safe_savgol_2d(density_z, filter_window, self.filter_order)
        vx_x      = _safe_savgol_2d(vx_x,      filter_window, self.filter_order)
        ...
        fill_val = np.mean(vx_x[mask]) if np.any(mask) else 0.0
        vx_x[density < threshold] = fill_val
    ...
    def pop_left_DF(self, new_start_time):
        # Trim raw DF history
        while len(self.time_log) > 0 and self.start_time < new_start_time:
            self.DF_log.popleft()
        ...
        # Trim interpolant history
        while len(self.time_interp) > 0 and self.interp_start < new_start_time:
            ...
            if len(self.time_interp) > 0:
                self.interp_start = self.time_interp[0]
            else:
                self.interp_start = new_start_time
                break
    ...
    def build_interpolant(self):
        ...
        """
        # Todo: check fill value
        # Todo: Important! consider faster 3D interpolation (Cython and parellel with prange, GIL release)
        # Todo: Probably accelerate trapz with jit (parallel)
        # Todo: Parallel cic with jit?
        # Todo: Also think about accelerate all numpy with numba.
        # Todo: Also consider GPU acceleration of trapz and scipy regulargridinterpolant with Cupy
        ...
        nx = len(self.time_interp)
        ny = self.x_grid_interp.shape[0]
        nz = self.z_grid_interp.shape[0]
    
        self.delta_x = (self.max_x - self.min_x) / (nx - 1) if nx > 1 else 0.0
        self.delta_y = (self.max_y - self.min_y) / (ny - 1) if ny > 1 else 0.0
        self.delta_z = (self.max_z - self.min_z) / (nz - 1) if nz > 1 else 0.0
