...
#    @profile
 ...
 #   @profile
  ...
  #  @profile
    ...
    def slope(self):
        """
        Robust slope calculation with fallback for SVD convergence failures
        """
        try:
            # Original method - works for normal cases
            p = np.polyfit(self.z, self.x, deg=1)
            return p
        except (np.linalg.LinAlgError, np.RankWarning) as e:
            # Fallback for SVD convergence failures or rank warnings
            print(f"Warning: polyfit failed ({e}), using robust fallback method")
            
            # Check data validity
            if len(self.z) < 2:
                print("Warning: Insufficient data points for slope calculation")
                return [0.0, np.mean(self.x) if len(self.x) > 0 else 0.0]
            ...
            if z_std < 1e-12:
                # All z values are essentially the same - no meaningful slope
                print("Warning: All z-coordinates are nearly identical, setting slope=0")
                return [0.0, np.mean(self.x)]
            
            if x_std < 1e-12:
                # All x values are the same - slope is 0
                print("Warning: All x-coordinates are nearly identical, setting slope=0")
                return [0.0, np.mean(self.x)]
            
            # Manual linear regression calculation (more robust than polyfit)
            z_mean = np.mean(self.z)
            x_mean = np.mean(self.x)
            
            # Calculate covariance and variance manually
            numerator = np.sum((self.z - z_mean) * (self.x - x_mean))
            denominator = np.sum((self.z - z_mean) ** 2)
            
            if abs(denominator) < 1e-12:
                print("Warning: Zero variance in z-coordinates, setting slope=0")
                return [0.0, x_mean]
            ...
            if not (np.isfinite(slope) and np.isfinite(intercept)):
                print("Warning: Non-finite slope/intercept, using fallback values")
                return [0.0, x_mean]
            
            print(f"Fallback calculation successful: slope={slope:.6e}, intercept={intercept:.6e}")
            return [slope, intercept]
        
        except Exception as e:
            # Ultimate fallback for any other unexpected errors
            print(f"Error: Unexpected failure in slope calculation ({e}), using safe defaults")
            x_mean = np.mean(self.x) if len(self.x) > 0 else 0.0
            return [0.0, x_mean]
