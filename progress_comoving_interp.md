...
#### 6ee. Prototype: O(1) lookup for non-uniform snapshot times (2026-09-14) ✅ **measurement overturned the planned design**

Gate before any production change: prove an O(1) non-uniform lookup works and is fast enough.
`pyDFCSR_2D/test/prototype_nonuniform_lookup.py`, standalone -- nothing in `pyDFCSR_2D`
imports it.

##### What has to be reproduced

The lookup locates the bracketing snapshot for a retarded time and returns an interpolation
weight; the caller forms `(1-a)*f[k] + a*f[k+1]`. Today, with uniform spacing, it is one
division (`interp3D.py:499-510`):
...
**SEGMENT (planned primary).** Store `(t0, dt, first, n)` per piecewise-uniform segment; scan
linearly for the segment, then one division inside it:

```
s = 0
while s < S-1 and q >= seg_t0[s+1]: s += 1
loc = (q - seg_t0[s]) / seg_dt[s]
k = seg_first[s] + floor(loc);   a = loc - floor(loc)
```

**BUCKET (planned fallback).** A uniform auxiliary index grid over the span, then a bounded
correction loop:
...
Building `j_k` from the runtime expression rather than from `t0 + j*h` is the load-bearing
detail: if build and runtime rounded differently, `j` could come out one too high, then
`t[bucket[j]] > q` and an increment-only loop could never recover -- wrong `k`, negative
weight. The real max-nodes-per-bucket is measured at build time and returned as the bound
rather than asserted in a comment.

**HYBRID (what the measurement selected).** One branch on a precomputed `is_uniform` flag:
uniform histories take the exact current expression, non-uniform take the bucket table.

##### Correctness

```
1. uniform nodes, segment path vs the production expression -- BIT-FOR-BIT
     n=50, n=1500, n=20000 : 0 mismatches over ~62k queries each
2. oracle equivalence (interpolated VALUE, not index), non-uniform nodes
     piecewise        segment 1.78e-14   bucket 0.00e+00
     one tiny gap     segment 2.13e-13   bucket 0.00e+00
     random 1e3       segment 0.00e+00   bucket 0.00e+00
3. overshoot: q one ulp either side of EVERY bucket edge
     piecewise  0 violations / 1559 queries;  random 1e3  0 / 3599
4. correction-loop bound vs spacing ratio (build-time bound vs observed)
     1e1  M=1540  (ideal 1540)    bound=1  observed=1  [uncapped]
     1e2  M=1600  (ideal 8422)    bound=3  observed=3  [capped]
     1e3  M=1600  (ideal 56500)   bound=7  observed=7  [capped]
     1e4  M=1600  (ideal 422408)  bound=9  observed=9  [capped]
5. n=2; q=t[0]; q=t[n-1] gives k=n-2 not n-1; 1-ulp gap gives a finite weight
```

The build-time bound is **exactly** predictive (observed == bound in all four rows), which is
the property the implementation relies on. Uncapped the bound is 1, better than the
theoretical 2. Capping `M` to keep the table cache-resident deliberately buys a longer loop
in exchange for fewer cache misses -- my first version of test 4 asserted `<= 2`
unconditionally and so failed on the capped rows; **the test was wrong, not the code**.

##### Efficiency, and the design reversal

ns per lookup, best of 3 over 2e6 lookups:

```
node set               S order         uniform   segment    bucket    hybrid    binary  srchsrt
uniform  n=20000       1 sorted           0.74      1.03      2.42      0.73     22.63    23.79
uniform  n=20000       1 random           0.74      1.03      9.50      0.73     36.77    38.47
piecewise S=3          3 sorted           0.75      1.63      1.47      1.47      7.65     8.27
piecewise S=3          3 random           0.74      4.19      3.03      3.02      8.40     9.71
piecewise S=8          6 sorted           0.74      1.83      1.47      1.47      7.45     8.15
piecewise S=8          6 random           0.74      6.72      2.98      2.97      9.79     9.76
random ratio 1e3    1499 random           0.75    196.58      3.26      3.32     26.49    25.52
```

**The planned per-segment primary is the wrong choice.** It is fine on sorted queries
(1.03-1.83 ns) but degrades to 4.19-6.72 ns on random ones, because the linear segment scan
defeats branch prediction, and it collapses to **196 ns** when the times are not segmentable.
The bucket table is flat at 1.47-3.48 ns in every case including 1499 segments.

**HYBRID wins on both counts**: 0.73 ns at `S == 1` against the 0.74 ns baseline -- i.e. the
bit-for-bit guarantee costs *nothing* -- and 1.47-3.49 ns otherwise. Binary search (7-38 ns)
and `np.searchsorted` (8-38 ns) are 10-50x worse, confirming they are unusable here.
...
about 1.5 %**. So the lookup was never going to be the bottleneck, and the right criterion is
robustness and the bit-for-bit guarantee rather than raw speed -- established by measurement
instead of assumed.

Query order measured at the same time: median **96.3 %** of adjacent query pairs
non-decreasing, but **0 %** of arrays fully sorted (min 49.3 %) -- consistent with a flattened
2D (x', s') grid that resets each row. So the operative benchmark columns are between sorted
and clustered, and a coherence optimisation (cache `k`, test `t[k] <= q < t[k+1]` first) has
little left to win given the 1.5 % headroom.

##### Recommendation for the production change

Ship the hybrid: `is_uniform` fast path plus the bucket table, **not** per-segment. Uniform
runs stay bit-for-bit identical at no cost, non-uniform runs pay ~1.5 % of wake time, and the
structure is insensitive to how the schedule is shaped -- which matters, because a scheduler
driven by waists and bend edges will not always produce cleanly segmentable times.

**Files.** `pyDFCSR_2D/test/prototype_nonuniform_lookup.py` (new, not imported by production).
